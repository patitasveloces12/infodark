...
function redirectToExtra(shopItem) {
    var destination = '/indexInternal.es?action=internalDock&tpl=internalDockSpecials';
...
//    };
    ...
    this.handleProfilePage = function(id, type, imgName) {
        ...
        xhr.onload = function(event){
            var responseArray = JSON.parse(event.target.response);
            var responseType = responseArray['type'];
            var responseError = responseArray['error'];
            var responseStatus = responseArray['status'];
            pilotSheet.handleFormUpload(responseType, responseError, responseStatus);
        };
    ...
    this.performProfileSearch = function() {
        ...
        var profileUsername;
        if (jQuery('#searchProfileField')) {
            profileUsername = jQuery('#searchProfileField').val();
        }
        ...
        jQuery.ajax({
            ...
            dataType: 'json',
            data: {command: 'searchProfileFromPilotProfile', profileUsername:profileUsername},
            success: function(response) {
                if (response.result == 'OK') {
                    pilotSheet.handleFormUpload("profileSearch", response.url, response.status);
                }
            },
...
function Achievements(){
    ...
    this.selectedCategory = function(id){
        ...
        }else if(id.indexOf('seasonpass') != -1){
            jQuery('#achievement_seasonpass').addClass('selectedCategory');
            jQuery('#' + id).addClass('selectedSubCategory');
